...
1. Use Xcode 27.0 / Swift 6.4. Run `swift test`, `scripts/build-app.sh debug`, `scripts/build-app.sh release`, and `scripts/verify-app.sh build/Export/Flycut.app`.
...
4. Run the reviewed commit through CI and manually dispatch **Build and Release**. Download the `Flycut-dmg` artifact. This signs and notarizes but does not publish.
...
```sh
codesign --verify --strict --verbose=2 Flycut.dmg
xcrun stapler validate Flycut.dmg
spctl -a -t open --context context:primary-signature -vv Flycut.dmg
