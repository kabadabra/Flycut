            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    if let report = model.report {
                        ...
                        Text("Preview compatibility: legacy menu row count, palette size, opacity, animation, single-click paste, clipboard-type reveal and automatic permission-alert suppression are retained in preferences but are not applied by this preview. Review these settings again before the final release.").font(.caption).foregroundStyle(.orange)
