            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    if let report = model.report {
                        ...
                        Text("Legacy menu previews become the initial palette rows; Show All and search reach every clipping. Palette dimensions fit the screen with a usable minimum. The legacy menu action controls double-click and accessible row activation; single-click selects and Return always pastes. Background opacity applies over native material. Accessibility reminders appear once per session unless suppressed.").font(.caption).foregroundStyle(.orange)
